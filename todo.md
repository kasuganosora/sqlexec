...
- [x] 自定义函数支持（已完成）
  ...
  - [x] 函数元数据管理
  - [x] SQL查询中调用自定义函数
    - 已实现：ExpressionEvaluator集成FunctionAPI
    - 支持：查询中调用用户注册的函数
  - [x] Go层面的函数注册系统
    - 已实现：FunctionAPI集成到表达式求值器
  - [x] 函数类型检查和转换
    - 已实现：参数类型验证和自动类型转换
  - [x] 聚合函数自定义扩展
    - 已实现：完整的聚合函数注册和构建接口
...
- [x] 实现具体的查询优化规则（谓词下推、JOIN重排序、列裁剪）
  - [x] 谓词下推：将 WHERE 条件下推到物理扫描阶段
  - [x] Limit 下推：将 LIMIT 下推到物理扫描阶段
  - [x] 列裁剪：减少不必要的数据传输
  - [x] JOIN 重排序：已实现规则（需要测试验证）
  - [x] 投影消除：已实现规则
  - [x] 常量折叠：已实现规则
  - [x] 半连接重写：已实现规则
