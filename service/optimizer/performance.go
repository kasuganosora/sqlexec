...
// NewPerformanceOptimizer 创建性能优化�?
...
func (so *ScanOptimization) Explain() string {
	if so.UseIndex {
		return fmt.Sprintf("建议使用索引 %s，预计扫�?%d �?, so.IndexName, so.EstimatedRows)
	}
	if so.PushDown {
		return fmt.Sprintf("建议将过滤条件下推，预计扫描 %d �?, so.EstimatedRows)
	}
	return "全表扫描"
}
