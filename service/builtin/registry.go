...
const (
	...
	ScopeUser   FunctionScope = "user"   // 用户自定义函�?
	ScopeSession FunctionScope = "session" // 会话函数
)

// FunctionMetadata 函数元数�?
type FunctionMetadata struct {
	Name        string            // 函数名（小写�?
	DisplayName string            // 显示名称
	Type        FunctionType      // 函数类型：标�?聚合/窗口
	Scope       FunctionScope     // 作用�?
...
type FunctionRegistryExt struct {
	...
	userFunctions   map[string]*FunctionMetadata // 用户自定义函�?
	sessionFunctions map[string]*FunctionMetadata // 会话函数
	aliases         map[string]string           // 函数别名
}

// NewFunctionRegistryExt 创建扩展函数注册�?
