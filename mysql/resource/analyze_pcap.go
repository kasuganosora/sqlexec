...
func main() {
	if len(os.Args) < 2 {
		fmt.Println("用法: go run analyze_pcap.go <pcapng文件>")
		os.Exit(1)
	}
	...
	for i := 0; i < len(data)-10; i++ {
		...
		if length > 0 && length < 100000 && i+4+length < len(data) {
			...
			if data[i+4] == 0x17 {
				...
				printPacket(packetData, found, i)
...
func printPacket(data []byte, index int, offset int) {
	if len(data) < 4 {
...
func parseCOMStmtExecute(payload []byte) {
	...
	// 分析 NULL bitmap 的计算公式
	fmt.Printf("\n    【NULL bitmap 长度分析】\n")
	for paramCount := 1; paramCount <= 20; paramCount++ {
		mysqlLen := (paramCount + 7) / 8
		mariadbLen := (paramCount + 2 + 7) / 8
		if mysqlLen == len(nullBitmap) || mariadbLen == len(nullBitmap) {
			fmt.Printf("      参数数=%d: MySQL=%d字节, MariaDB=%d字节\n",
				paramCount, mysqlLen, mariadbLen)
			if mysqlLen == len(nullBitmap) {
				fmt.Printf("        ✅ 可能是 MySQL 协议\n")
			}
			if mariadbLen == len(nullBitmap) {
				fmt.Printf("        ✅ 可能是 MariaDB 协议\n")
			}
		}
	}
	...
	if newParamsBindFlagOffset < len(payload) {
		...
		if newParamsBindFlag == 1 && len(payload) > newParamsBindFlagOffset+1 {
			...
			// 根据参数数量验证 NULL bitmap 长度
			if paramCount > 0 {
				mysqlLen := (paramCount + 7) / 8
				mariadbLen := (paramCount + 2 + 7) / 8
				fmt.Printf("      验证: MySQL=%d字节, MariaDB=%d字节, 实际=%d字节\n",
					mysqlLen, mariadbLen, len(nullBitmap))

				if len(nullBitmap) == mysqlLen && len(nullBitmap) != mariadbLen {
					fmt.Printf("      ⭐⭐⭐ 确认: 使用 MySQL 协议 (n+7)/8\n")
				}
				if len(nullBitmap) == mariadbLen && len(nullBitmap) != mysqlLen {
					fmt.Printf("      ⭐⭐⭐ 确认: 使用 MariaDB 协议 (n+2+7)/8\n")
				}
				if len(nullBitmap) == mysqlLen && len(nullBitmap) == mariadbLen {
					fmt.Printf("      ⚠️  两种协议在此场景结果相同，需要更多测试\n")
				}
			}
...
func parseParamValues(data []byte, nullBitmap []byte, paramCount int) {
	...
	for i := 0; i < paramCount && offset < len(data); i++ {
		...
		if mysqlIsNull || mariadbIsNull {
			...
			if mysqlIsNull && !mariadbIsNull {
				fmt.Printf("            → 确认使用 MySQL 协议\n")
			}
			if !mysqlIsNull && mariadbIsNull {
				fmt.Printf("            → 确认使用 MariaDB 协议\n")
			}
			continue
		}
